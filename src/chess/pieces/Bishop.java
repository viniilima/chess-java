...
public class Bishop extends ChessPiece {
	...
	public boolean[][] possibleMoves() {
		...
		// nw
		p.setValues(position.getRow() - 1, position.getColumn() - 1); // contador - acessa a posicao acima da peca. "position" vem da classe Piece
		while(getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)) { // enquanto a posicao p existir e nao houver uma peca na posicao atual
			...
			p.setValues(p.getRow() - 1, p.getColumn() - 1); // "aumenta" o contador
		}
